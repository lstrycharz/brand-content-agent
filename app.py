...
from agent import brand_voice as brand_voice_mod
...
tab_topics, tab_generate, tab_drafts, tab_settings = st.tabs(
    ["📋 Topics", "🚀 Generate", "📰 Drafts", "⚙️ Settings"]
)
...
def render_settings_tab() -> None:
    ...
    if current is None:
        ...
        with st.expander("Current brand guide", expanded=False):
            st.json(current)
    ...
    c1, c2 = st.columns([1, 1])
    if c1.button("✨ Generate brand guide", type="primary", use_container_width=True):
        if not description.strip():
            st.error("Description is empty.")
            return
        with st.spinner("Calling Claude..."):
            try:
                guide = brand_voice_mod.generate(description)
            except Exception as exc:  # noqa: BLE001
                st.error(f"Failed: {exc}")
                return
        st.success("Brand guide saved.")
        with st.expander("Generated guide", expanded=True):
            st.json(guide)

    if current is not None and c2.button(
        "🗑 Clear brand guide", use_container_width=True,
    ):
        brand_voice_mod.settings.brand_guide_path.unlink(missing_ok=True)
        st.success("Cleared. The agent will fall back to the default voice.")
        st.rerun()

    st.divider()
    st.subheader("Run log")
    log_path = brand_voice_mod.settings.run_log_path
    if log_path.exists():
        st.caption(f"`{log_path}` (auto-regenerated by the agent after every run)")
        st.markdown(log_path.read_text(encoding="utf-8"))
    else:
        st.info("No runs yet — generate an article to populate the log.")
